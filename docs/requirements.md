...
- AC-4: Every extracted entry keeps its archive-relative path; entries larger than 512 MB uncompressed (binary blobs such as dumpstate_board.bin ramdumps) are skipped with an announced progress message instead of exhausting memory (`bundle/a.log`, `bundle/sub/b.log`) and recurses; CRC32 digests defined by the archive are verified after decoding, so corrupt payloads surface a CRC error instead of silent garbage.
