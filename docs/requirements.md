...
- AC-5 (v1.48.0): At ≤ 560 px the viewer toolbar collapses to one row — quick filter, Mask toggle, and a ⋯ overflow button (`aria-expanded`-synced) that reveals the secondary controls (go-to-line, wrap, follow, view mode, copy + prefix, zen, drawer); the level-chips row becomes a single horizontally scrollable line; and the tab strip gains a right-edge fade as its scroll affordance (the version tag stays visible even at phone width — v1.49.1 revision of the v1.48.0 compaction). A fresh boot (no persisted state) at ≤ 560 px defaults Wrap ON, and while Wrap is off at ≤ 760 px the viewer shows a right-edge fade hinting horizontal overflow. The files overlay dims the content behind a tap-outside scrim and the panel header carries a ✕ close button; both close the overlay (restored sessions keep their persisted wrap choice at any width).
