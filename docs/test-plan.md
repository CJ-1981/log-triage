...
Version reference: v1.13.0. Development was test-driven and proceeded through quality gates G0–G8. Each gate has entry/exit criteria; coverage is mechanically enforced.
...
- **E2e:** Playwright suite **35/35 green** (27 app + 8 stress) locally (spec lists below).
...
| `npm run e2e` | Run the Playwright end-to-end suite (35 specs: 27 app + 8 stress) against the built `log-triage.html`. |
...
## E2e scope (Playwright — 35 specs: 27 app + 8 stress)

App suite (27 specs) — coverage includes:
...
For FR-25 (PII Providers tab, v1.14.0 target — design in ADR-0010), to land before or with the implementation:

- **Provider adapter tests** against a mock Presidio/LLM backend: request shape, findings normalized to `{line, start, end, type, score}`, timeout handling, and readable error surfacing for invalid endpoints.
- **Proxy URL handling**: proxy prefix applied only when the toggle is on; localhost Presidio requests unaffected; malformed proxy URL surfaces a readable error instead of a console trace.
- **Key non-export assertion**: Config-tab export output must never contain the API key (unit-level check plus an e2e download byte-check).
- **Warning banner**: visible whenever a remote (non-local) provider is active; absent for the local regex engine.
- **Provider switch semantics**: instant and per-load; the local regex engine remains the default after reload and works fully offline.
