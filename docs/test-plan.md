...
39. dlt-viewer text export (`tests/fixtures/dlt-viewer-sample.txt`, 42 messages): detected as format `dlt` (badge), chips count every level exactly (F1, V4 among them), the drawer on the VIN line shows tag `appid:ctid` and the session id as pid, masking applies to the raw VIN in the drawer, the sanitized `.log` export carries only the masked VIN, and the quick filter narrows to the bridge11 lines. Parser/detection semantics (including false-positive pins against logcat/syslog/ISO/CLF/MM-DD in both directions and the mode-word/args payload boundary) plus GNSS key-value pairs (v1.44.2: lat=/lon: token separators mask, non-coordinate words stay safe) live in the shared self-test cases (`[detect]`, `[ts]`, `[parse]` DLT entries).
