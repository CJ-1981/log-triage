...
## ADR-0015 — Streaming archive export: zip data descriptors, two-pass tar, 7z stays bounded

- **Status:** accepted (v1.45.0)
- **Context:** Archive export originally buffered every sanitized line per source file behind a 32 MiB limit and failed with “Archive exceeds the buffered limit” on larger extracts. Container formats constrain streaming: ZIP supports true streaming via data descriptors (local-header flag bit 3, sizes/CRC written after the data); a tar header must carry the entry size before the data; a 7z start header requires total pack/unpack sizes up front.
- **Decision:** New pure module `src/export-archive.js`: entries arrive as `(name, chunksFactory)` and nothing is buffered whole. ZIP streams in one pass with data descriptors and an incremental CRC; tar runs each entry’s factory twice (size pass, write pass); tar.gz pipes the tar byte stream through `CompressionStream('gzip')`. The worker gains `exportInit`/`exportPage` — a per-file filtered result set in a dedicated slot, so the live view’s order/revision are untouched while the filter scope is honored. `.7z` keeps the bounded buffered writer with an explicit message pointing at the streaming formats; selection-only archives (small by construction) stay buffered too.
- **Consequences:** Archive exports scale to any size when direct file saving is available (verified >32 MiB in e2e); the no-picker fallback stays capped at 32 MiB. Tar’s two-pass doubles the serialization (masking + worker paging) CPU for that format only. Entry order is per-file (name-sorted) rather than merged-timeline order — irrelevant inside an archive, where each entry is a separate file.
