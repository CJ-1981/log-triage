...
(function (root, factory) { if (typeof module === 'object' && module.exports) { module.exports = factory(); } else { Object.assign((root.LT || (root.LT = {})), factory()); } }(typeof self !== 'undefined' ? self : this, function () {
  ...
  async function deepScan() {
    ...
    $('search-progress').textContent = 'deep scan: ' + total + ' match(es)' + note;
    if (mode === 'count') {
  ...
  function jumpTo(idx) {
    switchTab('viewer');
    const v = viewer();
    const y = state.wrapOn ? (measureWrapOff() || 0) : idx * ROW_H;
    v.scrollTop = Math.max(0, y - v.clientHeight / 2);
    selection.click(idx);
    renderRows(); updateStatus();
    showDrawer(view[idx]);
  }
  function measureWrapOff() { // cheap jump in wrap mode
    if (!state.wrapOn) return 0;
    measureWrap();
    return heightSum[Math.min(idx0(), heightSum.length - 1)] || 0;
